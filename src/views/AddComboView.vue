    ...
    >
      ...
      <div class="addDishCon">
        ...
        <div class="ritCont">
          ...
          <div class="items">
            <div v-for="(item, index) in checkedList" :key="index" class="item">
              <span>{{ item.name }}</span>
              <span class="price">￥ {{ Number(item.price) / 100 }} </span>
              <span class="del" @click="delCheck(index)">
...
<script>
import { querySetmealById, addSetmeal, editSetmeal } from "@/api/combo";
...
export default {
  ...
  methods: {
    ...
    // 保存添加菜品列表
    addTableList() {
      this.dishTable = JSON.parse(JSON.stringify(this.checkedList));
      this.dialogVisible = false;
    },
      ...
      queryDishList({ categoryId: id }).then((res) => {
        if (res.data.code === 1) {
          if (res.data.data.length == 0) {
            ...
            this.dishAddList = res.data.data.forEach((item) => {
              item.dishId = item.id;
              item.copies = item.copies ? item.copies : 1;
            });
            this.dishAddList = res.data.data;
          }
        } else {
          this.$message.error(res.data.msg);
        }
